...
async def setup(app):
    ...
    @app.command("/slime_battleship")
    async def battleship_cmd(ack, command, client):
        await ack()
        uid = command["user_id"]; channel = command["channel_id"]
        text = (command.get("text") or "").strip()
        if channel in _games:
            await client.chat_postEphemeral(channel=channel, user=uid, text="a game is already running here — use `/slime_bs status` to see the board"); return
        import re as re_mod
        ...
        await client.chat_postMessage(channel=channel, text=f":anchor: *Battleship* started! {opponent_text}\n\nBoth players: place your ships using `/slime_bs place <col><row> <r|d>`\n*Ship sizes:* 5, 4, 3, 3, 2 — place them one at a time in that order\n\nExample: `/slime_bs place A0 r` places next ship starting at A0 going right\n\n{uid}: you go first. Place your 5-length ship.")
        if not is_ai:
            await client.chat_postMessage(channel=channel, text=f"<@{opponent_id}>: place your 5-length ship using `/slime_bs place <col><row> <r|d>` in this channel.")

    @app.command("/slime_bs")
    async def bs_cmd(ack, command, client):
        ...
        if action == "place":
            ...
            sub_parts = arg.split()
            if not sub_parts:
                await client.chat_postEphemeral(channel=channel, user=uid, text="usage: `/slime_bs place <col><row> [r|d]`  e.g. `/slime_bs place B3 r`"); return
            ...
            if all(game["setup_progress"][i] >= len(SHIP_SIZES) for i, s in enumerate(game["slots"]) if not s.is_ai):
                ...
                if not game["slots"][other_pidx].is_ai and game["setup_progress"][other_pidx] < len(SHIP_SIZES):
                    ...
                    await client.chat_postMessage(channel=channel, text=f"<@{game['slots'][other_pidx].user_id}>: place your {next_size}-length ship using `/slime_bs place <col><row> <r|d>`")
            ...
            if game["slots"][game["turn"]].is_ai:
                ...
                await client.chat_postMessage(channel=channel, text=f":dart: <@{next_slot.user_id}>'s turn! Use `/slime_bs fire <col><row>`")
            ...
            if phase == "battle":
                msg += ("\nAI is thinking..." if cur_slot.is_ai else f"\n<@{cur_slot.user_id}>'s turn — use `/slime_bs fire <col><row>`")
            await client.chat_postMessage(channel=channel, text=msg)
