...
async def setup(app):
    ...
    async def code_cmd(ack, command, client, body, respond):
        ...
        if action == "new":
            filename = arg
            if not filename:
                return await respond(text="Usage: `/slime_code new <filename>`", response_type="ephemeral")
            pad = _get_pad(uid)
            if filename in pad:
            ...
            filename = arg
            if not filename:
                return await respond(text="Usage: `/slime_code edit <filename>`", response_type="ephemeral")
            pad = _get_pad(uid)
            ...
            filename = arg
            if not filename:
                return await respond(text="Usage: `/slime_code view <filename>`", response_type="ephemeral")
            pad = _get_pad(uid)
            if filename not in pad:
            ...
            filename = arg
            if not filename:
                return await respond(text="Usage: `/slime_code delete <filename>`", response_type="ephemeral")
            pad = _get_pad(uid)
            if filename not in pad:
                return await respond(text=":x: File not found.", response_type="ephemeral")
            ...
            filename = arg
            if not filename:
                return await respond(text="Usage: `/slime_code run <filename>`", response_type="ephemeral")
            pad = _get_pad(uid)
            if filename not in pad:
                return await respond(text=":x: File not found.", response_type="ephemeral")
            code = pad[filename]
