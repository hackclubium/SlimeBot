    ...
    async def guild_cmd(ack, command, client):
        ...
        if action == "create":
            ...
            if not name or len(name) < 3 or len(name) > 32:
                await client.chat_postEphemeral(channel=channel, user=uid, text="usage: `/slime_guild create <name>` (3-32 chars)"); return
            ...
            try:
                ...
                await client.chat_postEphemeral(channel=channel, user=uid, text="usage: `/slime_guild join <guild_id>`"); return
            if user_guild_id(uid):
            ...
            if not gid:
                await client.chat_postEphemeral(channel=channel, user=uid, text="youre not in a guild — use `/slime_guild create <name>` or `/slime_guild join <id>`"); return
            guild_data = get_guild(gid)
            if not guild_data:
                await client.chat_postEphemeral(channel=channel, user=uid, text="that guild doesnt exist"); return
            ...
            try:
                ...
                await client.chat_postEphemeral(channel=channel, user=uid, text="usage: `/slime_guild deposit <amount>`"); return
            gid = user_guild_id(uid)
            if not gid:
                await client.chat_postEphemeral(channel=channel, user=uid, text="youre not in a guild"); return
