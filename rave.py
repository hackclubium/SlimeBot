...
async def setup(app):
    ...
    async def rave_cmd(ack, command, client):
        ...
        if sub == "bg":
            upload_key = arg
            if not upload_key:
                await client.chat_postEphemeral(channel=channel, user=uid, text="Usage: `/slime_rave bg <upload_key>` — share a video file and set its key here.")
                return
