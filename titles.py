...
async def setup(app):
    ...
    async def titles_cmd(ack, command, client):
        ...
        if action == "equip":
            title_input = arg
            if not title_input:
                await client.chat_postEphemeral(channel=channel, user=uid, text="usage: `/slime_titles equip <title name>`"); return
            ...
            msg = (
                ...
                f"{lines}\n\n_use `/slime_titles equip <title>` to equip one_"
            )
